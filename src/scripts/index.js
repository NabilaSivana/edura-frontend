...
const App = {
  ...
  currentRoute: null,
  currentContent: null,
  currentFullHash: '', // Track full hash including query params
  navigationState: {
    lastHash: '',
  ...
  // Helper to check if route needs dynamic re-rendering
  ...
  // Helper to get full route with query params for comparison
  getFullRouteKey() {
    const hash = window.location.hash;
    return hash; // Include full hash with query params
  },
  ...
  async renderPage() {
    ...
    const currentFullKey = this.getFullRouteKey();
    ...
    try {
      ...
      await this.renderNormalPageOptimized(main, navbarElement, currentRoute, currentFullKey);
  ...
  async renderNormalPageOptimized(main, navbarElement, currentRoute, currentFullKey) {
    const url = UrlParser.parseActiveUrlWithCombiner();
    const page = routes[url];

    // Enhanced caching logic - consider dynamic routes and full hash
    const shouldSkipRender = this.currentRoute === url && 
                            this.currentContent && 
                            this.currentFullHash === currentFullKey &&
                            !this.isDynamicRoute(url);

    if (shouldSkipRender) {
      console.log(`Skipping render for static route: ${url}`);
      return;
    }

    // Clear cache for dynamic routes or when full hash changes
    if (this.isDynamicRoute(url) || this.currentFullHash !== currentFullKey) {
      console.log(`Force re-render for dynamic route or hash change: ${url}`);
    ...
    try {
      ...
      if (typeof page.render === "function") {
        // For dynamic routes, always re-render
        if (this.isDynamicRoute(url)) {
          console.log(`Rendering dynamic route: ${url} with hash: ${currentFullKey}`);
        }
  ...
  // Method to force re-render for dynamic routes
  forceReRender() {
    this.currentRoute = null;
    this.currentContent = null;
    this.currentFullHash = '';
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("hashchange", (event) => {
    ...
    // Force re-render for certain routes that have query parameters
    const currentHash = window.location.hash;
    const isDynamicRoute = currentHash.includes('/course/session') || 
                          currentHash.includes('/course/flashcards') ||
                          currentHash.includes('/course/final-exam');
    
    if (isDynamicRoute) {
      console.log('Dynamic route detected, forcing re-render');
      App.forceReRender();
    }
...
// Custom event for forcing re-render
window.addEventListener('force-rerender', () => {
  App.forceReRender();
  debouncedRender();
});
