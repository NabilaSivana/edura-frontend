import '@fortawesome/fontawesome-free/css/all.min.css';import "../styles/style.css";import navbar from "./component/navbar.js";import routes from "./routes/route.js";import UrlParser from "./routes/url-parser.js";import NotFoundPage from "./utils/404.js";import UnauthorizedPage from "./utils/401.js";import ForbiddenPage from "./utils/403.js";import AuthGuard from "./utils/auth-guard.js";const App={isRendering:!1,currentRoute:null,currentContent:null,navigationState:{lastHash:'',lastNavbarState:null,lastSidebarState:null},initializeRoute(){let currentHash=window.location.hash;if(!currentHash||currentHash==='#'||currentHash===''){console.log('No hash found, redirecting to landing page');window.location.hash='#/';return!0}
return!1},async renderPage(){if(this.isRendering){return}
if(this.initializeRoute()){return}
const currentHash=window.location.hash;if(currentHash===this.navigationState.lastHash){return}
...
await this.updateNavbarOptimized(navbarElement,hideNavbarRoutes,currentHash);await this.renderNormalPageOptimized(main,navbarElement,currentRoute);this.navigationState.lastHash=currentHash}catch(error){console.error('Error in renderPage:',error);const main=document.querySelector("#main-content");if(main){this.showFallbackError(main,"Critical Error",error.message)}}finally{this.isRendering=!1}},async updateNavbarOptimized(navbarElement,hideNavbarRoutes,currentHash){if(!navbarElement)return;const shouldHide=hideNavbarRoutes.includes(currentHash);const currentNavbarState={hidden:shouldHide,hash:currentHash};if(this.navigationState.lastNavbarState&&this.navigationState.lastNavbarState.hidden===currentNavbarState.hidden&&this.navigationState.lastNavbarState.hash===currentNavbarState.hash){return}
...
this.navigationState.lastNavbarState=currentNavbarState},async renderNormalPageOptimized(main,navbarElement,currentRoute){const url=UrlParser.parseActiveUrlWithCombiner();const page=routes[url];if(this.currentRoute===url&&this.currentContent){return}
...
this.currentRoute=url;this.currentContent=main.innerHTML;return}
try{let newContent=null;if(typeof page.render==="function"){newContent=await page.render();if(newContent&&main.innerHTML!==newContent){const fragment=document.createDocumentFragment();const tempDiv=document.createElement('div');tempDiv.innerHTML=newContent;while(tempDiv.firstChild){fragment.appendChild(tempDiv.firstChild)}
...
this.currentRoute=url;this.currentContent=newContent||main.innerHTML}catch(error){console.error(`Error rendering page "${url}":`,error);this.showFallbackError(main,"Halaman Bermasalah",error.message)}},async renderAccessDeniedPage(reason,navbarElement,main){if(navbarElement&&navbarElement.style.display!=="none"){navbarElement.style.display="none";navbarElement.innerHTML=""}
...
if(typeof pageComponent.afterRender==='function'){try{await pageComponent.afterRender()}catch(error){console.error(`Error in ${reason} afterRender:`,error)}}}catch(error){console.error(`Error rendering access denied page for reason "${reason}":`,error);this.showFallbackError(main,"Access Denied",error.message)}},showFallbackError(container,title,message=null){const errorContent=`
    ...
    `;if(container.innerHTML!==errorContent){container.innerHTML=errorContent}},clearNavigationCache(){this.currentRoute=null;this.currentContent=null;this.navigationState={lastHash:'',lastNavbarState:null,lastSidebarState:null}}};let renderTimeout;let isInitialized=!1;const debouncedRender=()=>{if(renderTimeout){clearTimeout(renderTimeout)}
...
debouncedRender()});console.log("Initial page load, hash:",window.location.hash);debouncedRender()});window.addEventListener('unhandledrejection',(event)=>{console.error('Unhandled promise rejection:',event.reason);event.preventDefault()});window.addEventListener('error',(event)=>{console.error('JavaScript error:',event.error)});window.addEventListener('clear-navigation-cache',()=>{App.clearNavigationCache()});export default App