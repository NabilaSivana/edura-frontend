...
const DashboardTeacherPresenter = {
  async renderUnverifiedCourses(containerId) {
    ...
    try {
      ...
      if (!Array.isArray(courses) || courses.length === 0) {
        container.innerHTML = `<p class="text-center text-gray-600">Belum ada kursus yang menunggu verifikasi.</p>`;
        return;
      }
      ...
      container.innerHTML = `
       ...
       ${courses.map(course => `
            <div class="bg-white shadow rounded p-4 relative">
              <div class="absolute top-2 right-2 flex items-center text-sm text-gray-500">
              ...
              <p class="text-sm text-gray-600 mb-4 truncate">${course.description || "(Tidak ada deskripsi)"}</p>

              <div class="flex gap-2">
                <button class="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700" data-course-id="${course.id}">
                  Detail
                </button>
              </div>
            </div>
  ...
  async renderVerifiedCourses(containerId) {
    ...
    try {
      ...
      if (!Array.isArray(courses) || courses.length === 0) {
        container.innerHTML = `<p class="text-center text-gray-600">Belum ada kursus yang sudah diverifikasi.</p>`;
        return;
      }
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ${courses.map(course => `
            <div class="bg-white shadow rounded p-4 relative">
              ...
              <p class="text-sm text-gray-600 mb-4 truncate">${course.description || "(Tidak ada deskripsi)"}</p>
              <p class="text-xs text-gray-500 italic">Diverifikasi oleh: ${course.verified_by || "Admin"}</p>
            </div>
          `).join("")}
        </div>
      `;
