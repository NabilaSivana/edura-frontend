...
const CourseView = {
  render(courseData) {
    ...
    main.innerHTML = `
  <section class="min-h-screen max-w-4xl mx-auto p-6 bg-white dark:bg-gray-900 text-gray-800 dark:text-white rounded">

        <a href="#/dashboard" class="text-sm text-gray-600 dark:text-gray-300 hover:underline">&larr; Back to Dashboard</a>

        <!-- Banner + Progress -->
        <div class="flex items-center gap-6 border rounded p-6 mt-4 bg-white dark:bg-gray-800">
    ...
    sessions
      ...
      .forEach((s) => {
        ...
        card.className =
          "border rounded-md p-4 mb-3 shadow-sm bg-white dark:bg-gray-800";
  ...
  showError(message = "Terjadi kesalahan saat menampilkan konten.") {
    ...
    if (!main) return;
    main.innerHTML = `
      <div class="text-center p-10 text-red-500 dark:text-red-400">
        ${message}
      </div>
    `;
  },
