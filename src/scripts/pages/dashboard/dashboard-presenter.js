...
const DashboardPresenter = {
  async init() {
    ...
    try {
      ...
      user = await Api.getProfile();
      console.log('✅ Dashboard: User profile loaded:', user.full_name || user.name);
      
      // 🎯 DISPATCH EVENT UNTUK UPDATE NAVBAR SETELAH USER BERHASIL DIMUAT
      window.dispatchEvent(new CustomEvent('dashboard-user-loaded', {
        detail: {
          user: user,
