...
const RegisterPage = {
  async render() {
    return `
<style>
  /* === BAGIAN 1: GAYA DASAR (Sesuai Standar Login) === */
  .floating-input { position: relative; }
  .floating-input input {
    width: 100%; padding: 1rem 0.75rem 0.5rem; border: 1.5px solid #e5e7eb;
    border-radius: 0.75rem; background: white; color: #1f2937;
    font-size: 1rem; outline: none; transition: all 0.2s ease;
  }
  .floating-input input:focus {
    border-color: #2C2F8C; box-shadow: 0 0 0 3px rgba(44, 47, 140, 0.1);
  }
  ...
  /* Style Pesan Error, Success, Warning (Sesuai Login) */
  ...
  .password-toggle { position: absolute; right: 0.75rem; top: 50%; transform: translateY(-50%); cursor: pointer; color: #6b7280; }
...
<section class="block md:hidden w-screen min-h-screen bg-[#86A6DF] text-white font-sans overflow-y-auto">
  ...
  <div class="bg-white text-[#2C2F8C] rounded-t-[60px] px-8 py-10 flex flex-col items-center max-w-md mx-auto animate-slide-up">
    ...
    <p class="text-center mb-6">Buat akun Anda, silahkan isi form untuk mendaftar.</p>
    <div id="register-message-mobile" class="w-full max-w-sm"></div>
    <form id="register-form-mobile" class="w-full space-y-4">
      <div class="floating-input"><input type="text" id="name-mobile" placeholder=" " required /><label for="name-mobile">Nama Lengkap</label></div>
      <div class="floating-input"><input type="email" id="email-mobile" placeholder=" " required /><label for="email-mobile">Email</label></div>
      <div class="floating-input"><input type="password" id="password-mobile" placeholder=" " required /><label for="password-mobile">Password</label><i class="fas fa-eye absolute right-3 top-3 cursor-pointer text-gray-500" id="toggle-password-mobile"></i><div id="password-strength-mobile" class="password-strength"></div><div class="password-requirements"><div class="requirement" id="length-req-mobile"><i class="fas fa-circle"></i><span>Minimal 6 karakter</span></div><div class="requirement" id="uppercase-req-mobile"><i class="fas fa-circle"></i><span>Mengandung huruf besar</span></div><div class="requirement" id="number-req-mobile"><i class="fas fa-circle"></i><span>Mengandung angka</span></div></div></div>
      <div class="flex items-start"><input type="checkbox" id="terms-mobile" class="mr-2 mt-1" required /><label for="terms-mobile" class="text-sm text-gray-600">Saya setuju dengan <a href="#/terms" class="text-[#2C2F8C] hover:underline">Syarat dan Ketentuan</a></label></div>
      <button type="submit" class="bg-[#2C2F8C] text-white w-full py-2 rounded hover:bg-[#1e1f6c] transition"><span id="register-text-mobile">Daftar</span></button>
    </form>
  </div>
</section>
...
<section class="hidden md:flex w-screen h-screen font-sans m-0 p-0 overflow-hidden">
  ...
  <div class="w-1/2 bg-white flex flex-col justify-center items-center px-12 register-form-panel">
    <div class="w-full max-w-md">
      <div class="form-element-1">
        <h2 class="text-4xl font-extrabold text-[#2C2F8C] mb-2 text-center">Buat Akun Baru</h2>
        <p class="text-gray-600 text-center mb-6">Gabung dengan Edura dan mulai belajar!</p>
      </div>
      <div id="register-message-desktop" class="w-full form-element-2"></div>
      <form id="register-form-desktop" class="w-full space-y-4">
        <div class="floating-input form-element-2 enhanced-hover"><input type="text" id="name-desktop" placeholder=" " required /><label for="name-desktop">Nama Lengkap</label></div>
        <div class="floating-input form-element-3 enhanced-hover"><input type="email" id="email-desktop" placeholder=" " required /><label for="email-desktop">Email</label></div>
        <div class="floating-input relative form-element-4 enhanced-hover">
          <input type="password" id="password-desktop" placeholder=" " required />
          <label for="password-desktop">Password</label>
          <span class="password-toggle" id="toggle-password-desktop"><i class="fas fa-eye"></i></span>
          <div id="password-strength-desktop" class="password-strength"></div>
          <div class="password-requirements">
            <div class="requirement" id="length-req-desktop"><i class="fas fa-circle"></i><span>Minimal 6 karakter</span></div>
            <div class="requirement" id="uppercase-req-desktop"><i class="fas fa-circle"></i><span>Mengandung huruf besar</span></div>
            <div class="requirement" id="number-req-desktop"><i class="fas fa-circle"></i><span>Mengandung angka</span></div>
          </div>
        </div>
        <div class="flex items-start form-element-5"><input type="checkbox" id="terms-desktop" class="h-4 w-4 mt-1 mr-2 text-[#2C2F8C] focus:ring-[#2C2F8C] border-gray-300 rounded" required /><label for="terms-desktop" class="text-sm text-gray-600">Saya setuju dengan <a href="#/terms" class="text-[#2C2F8C] font-semibold hover:underline">Syarat & Ketentuan</a></label></div>
        <button type="submit" class="register-button ripple-button form-element-6 enhanced-hover"><span id="register-text-desktop">Daftar</span></button>
      </form>
    </div>
  </div>
</section>
    ...
    const handleRegisterSubmit = (formSelector, nameSelector, emailSelector, passwordSelector, termsSelector, messageElId, buttonTextId) => {
      ...
      form.addEventListener("submit", async (e) => {
        ...
        if (nameInput.value.length < 3) {
          ...
          return showMessage(messageElId, 'Nama lengkap minimal 3 karakter.', 'error');
        }
        ...
        RegisterPresenter.handleRegister(
          ...
          (errorMessage) => {
            ...
            if (errorMessage.includes('Email already registered')) {
              ...
              nameInput.classList.add('error');
              showMessage(messageElId, 'Nama lengkap minimal 3 karakter.', 'error');
            } else {
